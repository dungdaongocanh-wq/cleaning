...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        // Nếu format VN (dấu . ngăn cách nghìn, dấu , thập phân)
        // Thử parse lại đúng hơn:
        $rawPrice = trim($_POST['unit_price'] ?? '');
        $rawPrice = str_replace('.', '', $rawPrice); // bỏ dấu chấm ngăn cách nghìn
        $rawPrice = str_replace(',', '.', $rawPrice); // đổi dấu phẩy thập phân
        ...
        if (!$errors) {
            ...
            auditLog('create', 'customer_model_prices', $newId, null,
                ...
                "Thêm giá {$model['model_code']}: $price từ $fromDate"
            );
          ...
          <?php foreach ($priceList as $i => $p): ?>
            <tr <?= $i === 0 ? 'class="table-success"' : '' ?>>
              ...
              <td class="text-end fw-bold text-success">
                <?= number_format((float)$p['unit_price'], 0, ',', '.') ?> đ
              </td>
...
function openEditModal(id, fromDate, price, note) {
  ...
  document.getElementById('editUnitPrice').value = price;     // đã format
